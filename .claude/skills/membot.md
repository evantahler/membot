...
membot search "<question>" --rerank # + local cross-encoder precision pass (slower, higher precision)
```

`search` is the primary discovery tool — prefer it over scanning files. Add `--rerank` on a hard or ambiguous query to rescore the top candidates with a local cross-encoder.
...
- `search.rerank` (config key, default `false`) reranks every search with a local cross-encoder (`search.rerank_model`); the per-query `--rerank` flag (MCP: `rerank` parameter) overrides it. `search.max_per_file` (default `3`, `0` = unlimited) caps how many chunks from one file appear in results.
