...
`apple-notes:` reads `NoteStore.sqlite` directly via [`macos-ts`](https://www.npmjs.com/package/macos-ts) — no AppleScript, no browser. Grant **Full Disk Access** in System Settings → Privacy & Security → Full Disk Access to the app that launched membot — your terminal, editor, or agent app (Terminal, iTerm, Warp, Cursor, VSCode, Conductor, …) — then fully quit and relaunch it (one time). macOS attributes the grant to that host app, not to `membot` itself; the error message detects and names the exact app to add. The scope after the colon supports the same glob syntax as filesystem paths:
...
- **Config file:** `~/.membot/config.json` (optional; defaults are sane). Edit it directly or via `membot config`:
  ...
  **Cross-encoder reranking — what it is and why it helps.** Semantic search uses a **bi-encoder**: the query and every chunk are turned into vectors *independently* (chunks at ingest time, the query at search time), and results are ranked by cosine similarity between those vectors. That's what makes it fast and scalable — each chunk is embedded exactly once, and a search is one query-embedding plus a vector scan over the whole store. The catch is that the model never sees the query and a chunk *together*: each is compressed into a fixed 384-dim vector in isolation, so fine-grained relevance ("does this passage actually *answer* the question?" vs. merely "is it about the same topic?") gets flattened out. Bi-encoders are great at topical recall, weaker at pinpoint precision.

  A **cross-encoder** (the reranker) feeds the query and one candidate chunk through the model *as a single joined input* and emits one relevance score. Because every query token can attend to every chunk token, it captures relevance signals a bi-encoder structurally cannot. The price is that it can't be precomputed — you run a forward pass per `(query, chunk)` pair — so it's far too slow to score the whole store. membot uses the standard **retrieve-then-rerank** two-stage pattern: the fast bi-encoder + BM25 hybrid retrieves a shortlist of candidates, then the cross-encoder re-orders just that shortlist. You get bi-encoder recall over everything *and* cross-encoder precision on the finalists. In practice it most helps hard or ambiguous queries where several chunks are topically close — it pulls the one that genuinely answers the question to the top.

  `search.rerank` (default `false`) turns it on for every search; the per-query `membot search --rerank` flag (MCP: `rerank` parameter) overrides it. It's off by default because it adds query latency — and the first call downloads the ~23M-param cross-encoder (`search.rerank_model`, default `Xenova/ms-marco-MiniLM-L-6-v2`) into `~/.membot/models` — so reach for it when precision matters, not on every keystroke. `search.max_per_file` (default `3`, `0` = unlimited) caps how many chunks from one file appear in results, backfilling from other files so you still get `limit` hits.
