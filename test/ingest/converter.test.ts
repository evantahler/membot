...
describe("converter dispatch", () => {
	test("text/markdown passes through", async () => {
		const r = await convert(new TextEncoder().encode("# hi"), "text/markdown", "in", NO_LLM);
	...
	test("text/html → markdown via turndown", async () => {
		...
		const r = await convert(new TextEncoder().encode(html), "text/html", "in", NO_LLM);
	...
	test("convertHtml strips noscript", () => {
		const md = convertHtml(new TextEncoder().encode("<noscript>nope</noscript><p>ok</p>"));
		expect(md).toContain("ok");
		expect(md).not.toContain("nope");
	});

	test("unknown binary without API key produces a placeholder", async () => {
		const r = await convert(new Uint8Array([0, 1, 2, 3]), "application/x-blob", "src", NO_LLM);
		expect(r.markdown).toContain("unknown binary");
	});

	test("structured text without API key falls back to raw text", async () => {
		const json = `{"a": 1}`;
		const r = await convert(new TextEncoder().encode(json), "application/json", "src", NO_LLM);
